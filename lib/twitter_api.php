...
function twitter_api_login() {
	...
	if ($users) {
		...
		$consumer_secret = elgg_get_plugin_setting('consumer_secret', 'twitter_api');
                ...
                if ($code == 200) {
                    $twitter = json_decode($tmhOAuth->response['response']);
                }
                else {
		...
		$user = FALSE;
		if ($twitter_user = get_user_by_username($twitter->screen_name)) {
			if (($screen_name = $twitter_user->twitter_screen_name) && ($screen_name == $twitter->screen_name)) {
		...
		elgg_set_plugin_user_setting('twitter_name', $twitter->screen_name, $user->guid);
...
function twitter_api_get_access_token($oauth_verifier = FALSE) {
	...
	// fetch an access token
        ...
        $code = $tmhOAuth->request('POST',$tmhOAuth->url('oauth/access_token', ''),array('oauth_verifier' => $oauth_verifier));
